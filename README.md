...
survives across evenings. Banked stat points are spendable at the table now — the Rest scene draws
...
renderer to prove they stay on their feet, inside the frame, and able to get back up after a fall. One open defect. The generator used
to drop about 15% of manticore's barbed tail — it cropped the mesh to the largest connected
component, and the barbs are separate ones — and that is fixed upstream and rebuilt here. What is
left is a smaller, separate residual: roughly 1,400 interior pixels near the tail tip that the mesh
still loses, which keeps those four tiers just under the rest check's floor at 99.61 / 98.73 /
99.49 / 99.58% (art-pipeline §6.3). Anything that fails

...
`assets/manifest.json`'s `rigContract` and green through `art:verify:rig:strict`, so the story
lineup and the combat board both draw rigged figures rather than static PNGs. Anything that fails

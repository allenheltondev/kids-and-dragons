...
figures rather than static PNGs. Those 24 currently **fail** `art:verify:rig` on one check: they
were built on the 1024 art canvas, which leaves a knocked-down figure nowhere to topple into, and
the stage is now 1400. Their clip tables are correct; they need one regeneration pass, and the
client's rig anchor moves with it (art-pipeline §6.3). Anything that fails

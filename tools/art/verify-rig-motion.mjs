 ...
 *
 * The rigs are staged on an artboard LARGER than the art canvas so they have room
 * to rotate in (see any config's $stageComment), so a fixed 512/1024 would quietly
 * loosen every pixel threshold by the ratio between them. Read the stage size off
 * the configs — they all agree, and this fails loudly if they ever stop agreeing,
 * because one number here cannot describe two stages.
 */
const STAGE = (() => {
  const sizes = new Set(
 ...
 * hard-edged patches appearing and disappearing as the figure moves. The rest
 * check in `build-rigs.mjs` cannot see it, because it only ever looks at tick 0.
 *
 ...
 * pose rather than from the manifest origin. Same idea as build-rigs' rest
 * check: the rig at tick 0 of `idle` is the reference for everything else.
 */
...
if (rigJobs.length === 0) {
  console.error("error: no rigs found — run `npm run art:rigs` first");
  process.exit(2);
}

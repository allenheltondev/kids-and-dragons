 ...
 * read them from — and the moment one is added to the manifest it should be
 * deleted from here, which the run below checks for you.
 */
/**
...
// --- Effect sheets -----------------------------------------------------------
{
  ...
  lines.push({
    ...
    have: manifest.effects.length - missing.length,
    missing,
  });
...
// --- Biomes ------------------------------------------------------------------
{
  ...
  lines.push({
    ...
    need: manifest.biomes.length * 2,
    have: manifest.biomes.length * 2 - missing.filter((m) => !m.includes("props")).length,
    missing,
...
// --- Canonical entities ------------------------------------------------------
{
  ...
  lines.push({
    ...
    have: manifest.entities.length - missing.length,
    missing,
  ...
  lines.push({
    ...
    have: COMBAT_ROSTER.length - missingParts.length - stale.length,
    missing: missingParts.map((id) => (id === "will_o_wisp" ? `${id}  <- the only one shipped content fights` : id)),
    note: "asset-brief §9.5 — the prerequisite for enemy rigs; the other 18 entities need cutouts only",
  });
  if (stale.length > 0) {
    console.log(
      `  ${YELLOW}warn${RESET}  COMBAT_ROSTER names ${stale.join(", ")}, absent from manifest entities[]`,
    );
  }
}
...
for (const line of lines) {
  ...
  console.log(`  ${line.category.padEnd(pad)}  ${String(line.have).padStart(4)}/${String(line.need).padEnd(4)}  ${mark}`);

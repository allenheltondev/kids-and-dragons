...
> **The split has moved once since this was written: rigging is Claude's now.** It was scoped as
> Allen's — the one art step that stayed hands-on in the Rive editor — and it is now generated by
> `rive-mcp` from `assets/manifest.json`'s `rigContract`. Chapter 1 and Chapter 5 carry the struck
> line items. Art *direction* is unmoved: commissioning, the contact-sheet gate and the taste call
> are still Allen's, and rigging having a machine gate (`art:verify:rig`) is exactly why it could
> cross over and they cannot. See [art-pipeline §3](./art-pipeline.md#3-division-of-labor).

...
### 6.3 The delivered rigs, and the stage question still open
...
(§3). `art:verify:rig:strict` reads all 24 and passes.

**They are staged on the 1024 art canvas itself, and that is probably wrong.**

§3.1 rule 6 used to claim the 8px `edgeMarginPx` "guarantees room for rig rotation without
clipping". It does not, and the arithmetic is not close: 8px of margin is enough that a part is not
*born* against the edge and nowhere near enough for a character to rotate, leap or topple inside
one. A knocked-down figure needs room for its own **diagonal**, and a mythic tier fills the canvas
standing still. The contract ships `down` (6 ticks of fall), `down_loop` and `leap`, so this is not
hypothetical geometry — it is three clips per rig, on all 24.

The fix is a stage larger than the art: the 1024 canvas centred in a **1400×1400** artboard, 188px
of blank on every side, at scale 1, with the standing point still landing on the manifest origin so
rest stays 1:1 with `assembled.png` and `art-paths.ts`'s anchors stay true. That change is what
cleared every "off the artboard" failure and all 66 edge-grazing warnings when it was measured.

**It has not been applied here, deliberately.** Changing `artboardWidth/Height` in the configs
without regenerating the 24 `.riv` files would put the configs and the delivered rigs into
disagreement — and `verify-rig-motion.mjs` reads the stage size *off the configs* to set its pixel
scale, so it would then measure 1024-rendered rigs against 1400-sized thresholds and quietly loosen
every one of them. Config and rig have to move together, which means a rive-mcp regeneration pass.
Until then this section is the record, not the repo.

Two consequences to carry into that pass:

- **The client anchor changes.** `art-paths.ts` anchors static art at (0.5, 0.879) on 1024; a rig on
  1400 anchors at (0.5, 0.777). Same point in the world, different fraction of a different texture.
  Nothing renders rigs today so it costs nothing now — but the character then occupies ~73% of the
  linear texture it used to, and the §7 spike's offscreen buffer is sized for sharpness.

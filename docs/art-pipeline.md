    ...
    <species>/
      <tier>/                    fledgling | sworn | radiant | mythic
        ...
        rig.riv                  all 24 delivered, authored by rive-mcp from
                                 manifest.rigContract (§6.1). Currently RED through
                                 `art:verify:rig`: they were built on the 1024
                                 canvas and the stage is now 1400 (§6.3)
...
rigs are delivered. They are not currently green: the stage moved out from under them and they
need regenerating (§6.3).
...
parts are meshed — live in `art/rig/<species>.rig.json`. All 24 match the clip table; all 24
currently fail the artboard check, because the configs carry the 1400 stage and the built rigs do
not yet (§6.3).
...
(§3). All 24 were built on the bare 1024 art canvas, and **that stage is too small.**
...
**The six configs carry the new stage; the 24 `.riv` files do not yet.** They are stale builds, and
`art:verify:rig` now says so per rig rather than letting them pass: it reads each rig's actual
artboard through the runtime and fails any that disagrees with `rigStage`. That is why the gate is
red, and the remedy is a `rive-mcp` regeneration pass, not an edit to anything in this repo. The
stage check is kept out of `compareRigToContract` deliberately — a stale rig's clip table is
perfectly correct, and pointing the reader at the clip table would be pointing at the wrong fix.
...
- **The client's rig anchor is wrong the instant the stage changes, and nothing will catch it.**
  ...
  two different constants, and re-exporting one for the other stops being a convenience and becomes
  a bug. Regenerate the rigs and repoint `rive-rig.ts` at a stage-derived anchor in one change.
...
consistency test, then the other five species batched — 24 part-sets and 24 rigs. `art:verify` is
green; the rigs need one regeneration pass onto the 1400 stage (§6.3). Step 3 ("rig and animate it") turned out cheaper than
...
| ~~Binding tier skins to rigs~~ | ~~~15 min per tier~~ | **No longer human,** and cross-tier joint registration did hold — all four tiers of all six species are rigged. A regeneration onto the larger stage is outstanding (§6.3). |

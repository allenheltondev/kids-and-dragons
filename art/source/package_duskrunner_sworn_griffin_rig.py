#!/usr/bin/env python3
"""Build the Duskrunner Sworn Griffin exact-pose class-rig art package."""
...
def subject_alpha(portrait: Image.Image) -> Image.Image:
    ...
    # The garment and portrait backdrop share dark blue-purple values, so a
    # conservative cutoff punches holes in the hood and scarf. Keep the full
    # connected garment here; later steps constrain all extra alpha to the
    # measured gear envelope and preserve canonical anatomy independently.
...
def main() -> None:
    ...
    # Remove the portrait floor shadow and preserve the canonical feet.
    ...
    # The extracted matte may be conservative at pale feather edges. Restore
    # every canonical anatomy pixel, falling back to base art wherever chroma
    # removal clipped the approved portrait.

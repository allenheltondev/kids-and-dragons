#!/usr/bin/env python3
"""Build the Thornguard Mythic Manticore exact-pose class-rig art package."""
...
CANVAS = (1024, 1024)
SOURCE = ROOT / "assets/gear-portraits/thornguard/mythic/manticore.png"
BASE = ROOT / "assets/characters/manticore/mythic"
OUT = ROOT / "assets/character-rigs/thornguard/mythic/manticore"
PARTS = OUT / "parts"
REVIEW = ROOT / "art/review/thornguard_mythic_manticore_rig_split.png"
...
ARMOR_ENVELOPE = (125, 325, 700, 710)
...
def subject_alpha(portrait: Image.Image) -> Image.Image:
    """Remove the blue-purple backdrop while retaining Manticore and hawk."""
    ...
    for seed in ((480, 560), (585, 265)):
        component = foreground.copy()
        ImageDraw.floodfill(component, seed, 128, thresh=0)
        selected = np.maximum(
            selected, np.where(np.asarray(component) == 128, 255, 0).astype(np.uint8)
        )
...
def review_board(assembled: Image.Image) -> None:
    ...
    draw.text((50, 34), "Thornguard Mythic Manticore - rig split", font=title, fill="white")
    draw.text(
        (52, 88),
        "Approved exact pose   -   segmented tail remains rigged   -   hawk retained with armor assembly",
        font=note,
        fill="#b9c7d8",
    )
    panels = [
...
def main() -> None:
    ...
    hawk_region = (xx >= 470) & (xx < 700) & (yy >= 125) & (yy < 385)
    hawk_alpha = np.where(hawk_region, visible, 0).astype(np.uint8)
    armor_residual = np.where(hawk_region, 0, visible).astype(np.uint8)
    armor_alpha = keep_body_residual(parts, armor_residual, ARMOR_ENVELOPE)
    for name in BASE_PARTS:
        parts[name].save(PARTS / f"{name}.png")
    parts["armor_underlay"] = masked_portrait(portrait, armor_alpha)
    parts["armor_underlay"].save(PARTS / "armor_underlay.png")
    parts["hawk"] = masked_portrait(portrait, Image.fromarray(hawk_alpha, "L"))
    parts["hawk"].save(PARTS / "hawk.png")

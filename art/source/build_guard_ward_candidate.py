"""Build the review candidate for the eight-frame tintable guard_ward effect."""
...
SOURCE_DIR = ROOT / "art" / "source" / "guard_ward_candidate"
...
def render_frame(source: Image.Image, scale: float, opacity: float, dy: int) -> Image.Image:
    base_width = 128
...
def build_review(frames: list[Image.Image], output: Path) -> None:
    ...
    draw = ImageDraw.Draw(board)
    draw.text((margin, 22), "guard_ward — 8 frame review", font=load_font(28, True), fill=(244, 238, 222))
    draw.text(
        (margin, 61),
        "Protect transition · open shield bloom · runtime accent tint · 12 fps",
        font=load_font(17),
        fill=(169, 182, 204),
    )
...
def main() -> None:
    ...
    sheet.save(SOURCE_DIR / "guard_ward.sheet.png", optimize=True)

    build_review(frames, REVIEW_DIR / "guard_ward_candidate.png")
    build_gif(frames, REVIEW_DIR / "guard_ward_candidate.gif")

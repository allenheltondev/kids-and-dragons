...
PROP_NAMES = (
    "cloth_sail_windmill",
    "rope_bridge_anchor_gate",
    "floating_stepping_stones",
    "directional_wind_sail",
    "windswept_sky_pine",
    "temple_wind_waystone",
)
...
background_review = Image.new("RGB", (960, 590), (16, 29, 45))
background_review.paste(
    background.resize((960, 540), Image.Resampling.LANCZOS),
    (0, 0),
)
label_centered(background_review, "SKY ISLANDS", 550, 480)
background_review.save(REVIEW / "sky_islands_backdrop.png", optimize=True)

tile_sheet.save(REVIEW / "sky_islands_tiles.png", optimize=True)

props_review = Image.new("RGB", (512, 390), (16, 29, 45))
for prop_index, prop in enumerate(packaged_props):
    row, col = divmod(prop_index, 3)
    cell = Image.new("RGB", (160, 160), (35, 58, 75))
...
label_centered(props_review, "SKY ISLANDS", 350, 256)
props_review.save(REVIEW / "sky_islands_props.png", optimize=True)

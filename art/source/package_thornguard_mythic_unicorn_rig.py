#!/usr/bin/env python3
"""Build the Thornguard Mythic Unicorn exact-pose class-rig art package."""
...
CANVAS = (1024, 1024)
SOURCE = ROOT / "assets/gear-portraits/thornguard/mythic/unicorn.png"
BASE = ROOT / "assets/characters/unicorn/mythic"
OUT = ROOT / "assets/character-rigs/thornguard/mythic/unicorn"
PARTS = OUT / "parts"
REVIEW = ROOT / "art/review/thornguard_mythic_unicorn_rig_split.png"
BASE_PARTS = ("tail", "leg_l", "leg_r", "body", "arm_l", "arm_r", "head", "mane", "horn")
Z_ORDER = (*BASE_PARTS, "armor_visible")
ARMOR_ENVELOPE = (250, 430, 665, 680)
...
def subject_alpha(portrait: Image.Image) -> Image.Image:
    """Remove the smooth navy backdrop while preserving the approved silhouette."""
    ...
    ImageDraw.floodfill(foreground, (500, 500), 128, thresh=0)
    return Image.fromarray(np.where(np.asarray(foreground) == 128, 255, 0).astype(np.uint8), "L")
...
def review_board(assembled: Image.Image) -> None:
    ...
    draw.text((50, 34), "Thornguard Mythic Unicorn - rig split", font=title, fill="white")
    draw.text(
        (52, 88),
        "Approved exact pose   -   armor follows registered anatomy   -   mane and horn retain authored overlap",
        font=note,
        fill="#b9c7d8",
    )
    panels = [

#!/usr/bin/env python3
"""Build the Thornguard Mythic Bigfoot exact-pose class-rig art package."""
...
CANVAS = (1024, 1024)
SOURCE = ROOT / "assets/gear-portraits/thornguard/mythic/bigfoot.png"
BASE = ROOT / "assets/characters/bigfoot/mythic"
OUT = ROOT / "assets/character-rigs/thornguard/mythic/bigfoot"
PARTS = OUT / "parts"
REVIEW = ROOT / "art/review/thornguard_mythic_bigfoot_rig_split.png"
BASE_PARTS = ("leg_l", "leg_r", "body", "arm_l", "arm_r", "head", "mane")
Z_ORDER = ("leg_l", "leg_r", "body", "arm_l", "arm_r", "head", "armor_visible", "mane")
ARMOR_ENVELOPE = (295, 350, 765, 700)
...
def subject_alpha(portrait: Image.Image) -> Image.Image:
    """Remove the smooth navy backdrop and retain the connected Bigfoot silhouette."""
    ...
    connected = Image.fromarray(np.where(residual > 8, 255, 0).astype(np.uint8), "L")
...
def review_board(assembled: Image.Image) -> None:
    ...
    draw.text((50, 34), "Thornguard Mythic Bigfoot - rig split", font=title, fill="white")
    draw.text(
        (52, 88),
        "Approved exact pose   -   armor follows registered anatomy   -   mane deliberately remains above armor",
        font=note,
        fill="#b9c7d8",
    )
    panels = [

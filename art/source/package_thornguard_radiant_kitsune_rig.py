#!/usr/bin/env python3
"""Build the Thornguard Radiant Kitsune exact-pose class-rig art package."""
...
CANVAS = (1024, 1024)
SOURCE = ROOT / "assets/gear-portraits/thornguard/radiant/kitsune.png"
BASE = ROOT / "assets/characters/kitsune/radiant"
OUT = ROOT / "assets/character-rigs/thornguard/radiant/kitsune"
PARTS = OUT / "parts"
REVIEW = ROOT / "art/review/thornguard_radiant_kitsune_rig_split.png"
BASE_PARTS = ("tail", "leg_l", "leg_r", "body", "arm_l", "arm_r", "head", "mane")
# The saddle, armor overhang and squirrel ride with the body. The neck mane is
# drawn last so it retains the approved overlap along the breastplate edge.
Z_ORDER = ("tail", "leg_l", "leg_r", "body", "arm_l", "arm_r", "head", "armor_visible", "mane")
REGISTER_Y = -16
ARMOR_ENVELOPE = (120, 170, 681, 706)
...
def subject_alpha(portrait: Image.Image) -> Image.Image:
    """Remove the smooth backdrop while retaining Kitsune and squirrel."""
    ...
    foreground = Image.fromarray(np.where(residual > 8, 255, 0).astype(np.uint8), "L")
    foreground = foreground.filter(ImageFilter.MaxFilter(3)).filter(ImageFilter.MinFilter(3))

    selected = np.zeros((CANVAS[1], CANVAS[0]), dtype=np.uint8)
    for seed in ((475, 540), (485, 285)):
...
def review_board(assembled: Image.Image) -> None:
    ...
    draw.text((50, 34), "Thornguard Radiant Kitsune - rig split", font=title, fill="white")
    draw.text(
        (52, 88),
        "Approved pixels on exact rig masks   -   saddle follows body   -   squirrel retained with the armor assembly",
        font=note,
        fill="#b9c7d8",
    )
    panels = [

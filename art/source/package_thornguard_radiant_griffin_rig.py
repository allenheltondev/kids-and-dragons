#!/usr/bin/env python3
"""Build the Thornguard Radiant Griffin exact-pose class-rig art package."""
...
CANVAS = (1024, 1024)
SOURCE = ROOT / "assets/gear-portraits/thornguard/radiant/griffin.png"
BASE = ROOT / "assets/characters/griffin/radiant"
OUT = ROOT / "assets/character-rigs/thornguard/radiant/griffin"
PARTS = OUT / "parts"
...
def subject_alpha(portrait: Image.Image) -> Image.Image:
    """Remove the smooth backdrop and the separate hummingbird component."""
    ...
    background_samples = (xx < 90) | (xx > 933) | (yy < 90) | (yy > 933)
    ...
    hummingbird = (xx >= 625) & (yy < 365)
    mask = ~hummingbird & (residual > 8)
    ...
    # Retain only the connected component containing the approved harness;
    # detached residuals are backdrop texture, never part of the Griffin.
    ImageDraw.floodfill(connected, (500, 550), 128, thresh=0)
...
def review_board(assembled: Image.Image) -> None:
    ...
    draw.text((50, 34), "Thornguard Radiant Griffin — rig split", font=title, fill="white")
    draw.text(
        (52, 88),
        "Approved Griffin silhouette only   •   hummingbird excluded   •   folded armored wing follows wing bone",
        font=note,
        fill="#b9c7d8",
    )
    panels = [

#!/usr/bin/env python3
"""Build the Thornguard Mythic Griffin exact-pose class-rig art package."""
...
CANVAS = (1024, 1024)
SOURCE = ROOT / "assets/gear-portraits/thornguard/mythic/griffin.png"
BASE = ROOT / "assets/characters/griffin/mythic"
OUT = ROOT / "assets/character-rigs/thornguard/mythic/griffin"
PARTS = OUT / "parts"
REVIEW = ROOT / "art/review/thornguard_mythic_griffin_rig_split.png"
BASE_PARTS = ("wings", "tail", "leg_l", "leg_r", "body", "arm_l", "arm_r", "head", "mane")
Z_ORDER = (*BASE_PARTS, "armor_visible")
ARMOR_ENVELOPE = (205, 270, 765, 705)
...
def subject_alpha(portrait: Image.Image) -> Image.Image:
    """Remove the smooth backdrop and the separate hummingbird companion."""
    ...
    connected = Image.fromarray(
        np.where(~hummingbird & (residual > 8), 255, 0).astype(np.uint8), "L"
    )
...
def review_board(assembled: Image.Image) -> None:
    ...
    draw.text((50, 34), "Thornguard Mythic Griffin - rig split", font=title, fill="white")
    draw.text(
        (52, 88),
        "Approved Griffin silhouette only   -   hummingbird excluded   -   armor follows registered anatomy",
        font=note,
        fill="#b9c7d8",
    )
    panels = [

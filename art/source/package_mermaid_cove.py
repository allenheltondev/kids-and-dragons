...
PROP_NAMES = (
    "flowering_reef_arch",
    "coral_garden",
    "tropical_palm_cluster",
    "shell_tide_pool",
    "ancient_ocean_marker",
    "waterfall_ledge",
)
...
column_edges = (
    (
        0,
        round(prop_source.width * 0.400),
        round(prop_source.width * 0.671),
        prop_source.width,
    ),
    (
        0,
        round(prop_source.width * 0.361),
        round(prop_source.width * 0.609),
        prop_source.width,
    ),
)
for index, name in enumerate(PROP_NAMES):
    row, col = divmod(index, 3)
    left = column_edges[row][col]
    right = column_edges[row][col + 1]
    top = row_edges[row]
...
label_centered(background_review, "MERMAID COVE", 550, 480)
background_review.save(REVIEW / "mermaid_cove_backdrop.png", optimize=True)

tile_sheet.save(REVIEW / "mermaid_cove_tiles.png", optimize=True)
...
label_centered(props_review, "MERMAID COVE", 350, 256)
props_review.save(REVIEW / "mermaid_cove_props.png", optimize=True)

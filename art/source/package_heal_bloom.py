...
SOURCE = ROOT / "art" / "source" / "heal_bloom_storyboard_raw.png"
...
def to_transparent_effect(cell: Image.Image, remove_edge_spill: bool = False) -> Image.Image:
    ...
    scale = min(0.72, 236 / max(effect.size))
...
frames = []
row_edges = (0, 380, 800, source.height)
    ...
    for col in range(COLS):
        ...
        frames.append(
            to_transparent_effect(
                ...
                remove_edge_spill=frame_index == 6,
            )
        )
...
strip.save(EFFECTS / "heal_bloom.sheet.png", optimize=True)

(EFFECTS / "heal_bloom.json").write_text(
...
review.save(REVIEW / "heal_bloom_frames.png", optimize=True)

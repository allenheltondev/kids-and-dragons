...
PROP_NAMES = (
    ...
    "cliffside_harbor_crane",
    "mosaic_trade_waystone",
    "merchant_handcart",
    "harbor_signal_bell_mast",
)
...
background_review = Image.new("RGB", (960, 590), (12, 25, 41))
background_review.paste(
    background.resize((960, 540), Image.Resampling.LANCZOS),
    (0, 0),
)
label_centered(background_review, "THE EXCHANGE", 550, 480)
background_review.save(REVIEW / "exchange_backdrop.png", optimize=True)

tile_sheet.save(REVIEW / "exchange_tiles.png", optimize=True)

props_review = Image.new("RGB", (512, 390), (12, 25, 41))
for prop_index, prop in enumerate(packaged_props):
    row, col = divmod(prop_index, 3)
    cell = Image.new("RGB", (160, 160), (35, 53, 66))
...
label_centered(props_review, "THE EXCHANGE", 350, 256)
props_review.save(REVIEW / "exchange_props.png", optimize=True)
